...
export default function BoilPage() {
  ...
  // faucet transition and stove transitions
  ...
  // typewriter effects, and actual boiling instruction portion
  ...
  // shake effect for pot with lid
  const [shakeIndex, setShakeIndex] = useState(0);
  const [shakeCount, setShakeCount] = useState(0);

  // results and back button
  ...
  // mainly for the typewriter effect
  const indexRef = useRef(0);

  function getInstructionForTime(time: number) {
    if (time < 1) return ' Boiling an egg for less than a minute won’t cook it as it’ll still be raw inside. At best, you’ve just given it a steamy wake-up call.';
    if (time > 10) return ' Boil an egg too long and the yolk goes dry with a gray ring like it’s aged ten years in ten minutes. Congratulations, you’ve just overcooked breakfast into a biology experiment.';
    return '';
  }
  ...
  // faucet transition and filling pot with water
  useEffect(() => {
    ...
    if (step === 3) {
      ...
      timeout1 = setTimeout(() => {
        ...
        timeout2 = setTimeout(() => {
          ...
          setShowPotWaterSVG(true); // show pot with water
  ...
  useEffect(() => {
    if (showPotWithLid) {
      const count = Math.min(5, Math.floor(cookTime)); // cap at 5 shakes max
      setShakeCount(count);
      setShakeIndex(0);

      setShakeIndex(0);
    }
  }, [showPotWithLid, cookTime]);
